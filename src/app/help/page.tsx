...
import { TrendingUp, Shield, ArrowUpCircle, HelpCircle } from 'lucide-react'
...
const FinancialGuide = () => {
  const metrics = {
    profitability: {
      ...
      icon: TrendingUp,
      description:
        '회사가 얼마나 돈을 잘 버는지 보여주는 지표예요 (1분기 누적 데이터 기준)',
      metrics: [
        {
          name: '영업이익률',
          formula: '영업이익(누적) ÷ 매출액(누적) × 100',
          description:
            '본업에서 얼마나 돈을 잘 버는지 알려주는 지표예요 (1분기 누적 기준)',
          goodRange: '10% 이상이면 좋음',
          example: '영업이익률 15%면 매출 100원당 15원의 이익을 남긴다는 뜻',
        },
        {
          name: '순이익률',
          formula: '당기순이익(누적) ÷ 매출액(누적) × 100',
          description:
            '최종적으로 얼마나 돈을 벌었는지 보여주는 지표예요 (1분기 누적 기준)',
          goodRange: '5% 이상이면 좋음',
